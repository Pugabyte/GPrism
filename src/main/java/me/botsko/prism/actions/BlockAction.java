...
public class BlockAction extends GenericAction {
	...
	public void setBlock(BlockState state) {
		if (state != null) {
			...
			// signs
			else if (MaterialTag.SIGNS.isTagged(state.getType())) {
	...
	public void deserialize(String data) {
		if (data != null && data.startsWith("{")) {
			...
			else if (MaterialTag.SIGNS.isTagged(getMaterial())) {
		...
		if (!is_preview) {
			...
			if ((getMaterial() == Material.PLAYER_HEAD || getMaterial() == Material.PLAYER_WALL_HEAD)
				...
				if (!s.owner.isEmpty()) {
					...
					skull.setOwningPlayer(Bukkit.getOfflinePlayer(UUID.fromString((s.owner))));
				}
			...
			if (parameters.getProcessType() == PrismProcessType.ROLLBACK
					&& MaterialTag.SIGNS.isTagged(getMaterial())
			...
			boolean physics = true; // !parameters.hasFlag(Flag.NO_PHYS);
			...
			// Preview it
			player.sendBlockChange(block.getLocation(), getBlockData());
			...
			for (final CommandSender sharedPlayer : parameters.getSharedPlayers()) {
				if (sharedPlayer instanceof Player) {
					((Player) sharedPlayer).sendBlockChange(block.getLocation(), getBlockData());
				}
			}
		}
	...
	protected ChangeResult removeBlock(Player player, QueryParameters parameters, boolean is_preview, Block block) {
		...
		if (!block.getType().equals(Material.AIR)) {
			...
			if (!is_preview) {
				...
				for (final CommandSender sharedPlayer : parameters.getSharedPlayers()) {
					if (sharedPlayer instanceof Player) {
						((Player) sharedPlayer).sendBlockChange(block.getLocation(), Bukkit.createBlockData(Material.AIR));
					}
				}
			}
			return new ChangeResult(ChangeResultType.APPLIED, stateChange);
