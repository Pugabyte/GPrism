	...
	protected void showLocationHistory(final Player player, final Location loc) {
		...
		plugin.getServer().getScheduler().runTaskAsynchronously(plugin, () -> {
			...
			if (params.getActionTypes().size() == 0) {
				@SuppressWarnings("unchecked") final ArrayList<String> ignoreActions = (ArrayList<String>) plugin.getConfig().getList(
			...
			if (!results.getActionResults().isEmpty()) {
				final String blockname = block.getType().toString().toLowerCase();
