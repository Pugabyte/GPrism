...
public class WandCommand implements SubHandler {
	...
	public void handle(CallInfo call) {
		...
		String configMaterial = null;
		if ("item".equals(mode)) {
			configMaterial = plugin.getConfig().getString("prism.wands.default-item-mode-id");
		} else if ("block".equals(mode)) {
			configMaterial = plugin.getConfig().getString("prism.wands.default-block-mode-id");
		}
		...
		if (plugin.getConfig().getBoolean("prism.wands.allow-user-override")) {
			...
			if (personalToolKey != null) {
				configMaterial = personalToolKey;
			}
		}

		if (configMaterial != null) {
			material = Material.matchMaterial(configMaterial);
		...
		if (!Material.AIR.equals(material)) {
			wandOn += " on a " + ItemUtils.getNiceName(material);
		}
		...
		if (ItemUtils.isAcceptableWand(material)) {
		...
		if (enabled) {
			...
			wand.setItemType(material);

			Prism.debug("Wand activated for player - mode: " + mode + " Item:" + material);

			// Move any existing item to the hand, otherwise give it to them
			if (plugin.getConfig().getBoolean("prism.wands.auto-equip")) {
				if (!InventoryUtils.moveItemToHand(inv, material)) {
					...
					if (InventoryUtils.handItemToPlayer(inv, new ItemStack(material))) {
