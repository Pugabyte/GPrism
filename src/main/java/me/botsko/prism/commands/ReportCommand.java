	...
	protected void blockSumReports(final CallInfo call) {
		...
		plugin.getServer().getScheduler().runTaskAsynchronously(plugin, () -> {
			...
			try {
				...
				call.getSender().sendMessage(
						Prism.messenger.playerHeaderMsg("Total block changes fork " + ChatColor.DARK_AQUA
								+ playerName));
				call.getSender().sendMessage(
				...
				while (rs.next()) {
					...
					final String alias = ItemUtils.getNiceName(Material.matchMaterial(rs.getString(1)));
