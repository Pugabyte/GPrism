...
public class SetmyCommand implements SubHandler {
	...
	public void handle(CallInfo call) {
		...
		if (setWandItem != null) {
			...
			Material material = Material.matchMaterial(setWandItem);
			if (material == null) {
				call.getPlayer().sendMessage(Prism.messenger.playerError("There's no item matching that name."));
				return;
			}

			String itemName = ItemUtils.getNiceName(material);
			...
			Settings.saveSetting("wand.item", material.name(), call.getPlayer());
			call.getPlayer().sendMessage(
